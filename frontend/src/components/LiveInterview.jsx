...
export default function LiveInterview({ user, navigateToDashboard, interviewId }) {
  ...
  const startScreenShare = async () => {
    try {
      ...
      socket.emit("screen-share-start", { roomId: roomIdRef.current });
  ...
  const handleJoinRoom = () => {
    ...
    setPhase("waiting");
    setMessages([{ id: 1, sender: "system", text: `You joined session "${interviewId}".` }]);
  };
  ...
  const handleSendMessage = () => {
    ...
    const msg = { id: Date.now(), sender: getMySenderName(), text: chatInput.trim() };
    setMessages(prev => [...prev, msg]);
    socket.emit("chat-message", { roomId: interviewId, message: msg });
    setChatInput("");
  };
  ...
  const handleCodeChange = (val) => {
    ...
    setEditorCode(val);
    socket.emit("code-update", { roomId: interviewId, code: val });
  };
  ...
  const handleMcqClick = (optId) => {
    ...
    setMcqFeedback(null);
    socket.emit("mcq-update", { roomId: interviewId, optionId: optId });
    socket.emit("candidate-activity", { roomId: interviewId, activity: `Selected Option ${optId} for MCQ` });
  };
  ...
  const handleSectionChange = (section) => {
    ...
    if (!isInterviewer) {
      ...
      socket.emit("candidate-activity", { roomId: interviewId, activity: label });
    }
  };
  ...
  const handleRunCode = async () => {
    ...
    setIsRunning(true);
    setOutput("Executing...\n");
    socket.emit("candidate-activity", { roomId: interviewId, activity: "Running code..." });
    ...
    try {
      ...
      if (res.ok) {
        ...
        socket.emit("candidate-activity", { roomId: interviewId, activity: `Code executed — ${data.result.error ? "Error" : "Success"}` });
      } else {
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#0a0a0a", padding: "24px" }}>
        <Card style={{ backgroundColor: "#1e1e1e", border: "1px solid #333", width: "100%", maxWidth: "480px" }}>
          <CardHeader style={{ textAlign: "center", paddingBottom: "8px" }}>
            <CardTitle style={{ fontSize: "28px", fontFamily: "'Space Grotesk', sans-serif", marginBottom: "8px" }}>Live Interview Session</CardTitle>
          ...
          <CardContent style={{ display: "flex", flexDirection: "column", gap: "20px", paddingTop: "16px" }}>
            ...
            <Button
              ...
              disabled={!interviewId || !permissionsGranted}
        ...
        <div className="demo-header">
          <div className="demo-header-left">
            ...
            <div className="demo-title">
              ...
              <span className="demo-badge" style={{ backgroundColor: "#D97706" }}>SESSION {interviewId}</span>
            </div>
          </div>
      ...
      <div className="demo-header">
        <div className="demo-header-left">
          ...
          <div className="demo-title">
            ...
            <span className="demo-badge" style={{ backgroundColor: "#22C55E" }}>SESSION {interviewId}</span>
          </div>
        </div>
        <div className="demo-header-right">
          <div className="demo-timer">Time: <span className="mono-time">{formatTime(elapsedTime)}</span></div>
          ...
          {!isSubmitted ? (
            <Button variant="default" style={{ backgroundColor: "#D97706", color: "#171717" }} onClick={() => {
              setIsSubmitted(true);
              socket.emit("candidate-activity", { roomId: interviewId, activity: "Submitted all answers" });
            }}>
              Submit All
            </Button>
          ) : (
            ...
            <>
              <div className="editor-toolbar">
                <div style={{ display: "flex", alignItems: "center", gap: "24px" }}>
                  ...
                  <button className="reset-editor-btn" onClick={() => { const s = MOCK_CODING_QUESTION.starterCode; setEditorCode(s); socket.emit("code-update", { roomId: interviewId, code: s }); }} title="Reset code">
                    <Icon.Refresh />
                  </button>
                </div>
                <div style={{ display: "flex", gap: "16px" }}>
